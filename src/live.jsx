...
export function LiveProvider({ children }) {
  ...
  const extraSymbols = useRef(new Set());
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const requestedSymbols = [...new Set([...API_SYMBOLS, ...extraSymbols.current])].slice(0, 25);
        const r = await fetch(
  ...
  const api = {
    ...
    watchSymbol(ticker) {
      ...
      if (!/^[A-Z][A-Z0-9.-]{0,9}$/.test(symbol) || symbol === "BTC") return;
      extraSymbols.current.add(symbol);
      if (!dataRef.current[symbol]) {
        dataRef.current[symbol] = {
          price: 0, open: 0, spark: [], dataState: "loading",
          providerTimestamp: null, freshness: "unknown", sessionState: "unknown",
        };
      }
    },
  };
